...
clientSideTransformer = (xmlCode, xsltCode) ->
  `var sheetDoc`
  `var inputDoc`
  if typeof XSLTProcessor != 'undefined' and typeof DOMParser != 'undefined'
    processor = new XSLTProcessor
    domParser = new DOMParser
    sheetDoc = domParser.parseFromString(xsltCode, 'application/xml')
    processor.importStylesheet sheetDoc
    inputDoc = domParser.parseFromString(xmlCode, 'application/xml')
    ...
    try
      sheetDoc = new ActiveXObject('Msxml2.DOMDocument.6.0')
      sheetDoc.loadXML xsltCode
      inputDoc = new ActiveXObject('Msxml2.DOMDocument.6.0')
      inputDoc.loadXML xmlCode
      return inputDoc.transformNode(sheetDoc)
    catch e
      return e.message
  return
  
transform = () ->
  if $('#engine').val() == 'Browser'
    resultString = clientSideTransformer($('#xml').val(), $('#xsl').val())
    resulteditor.getSession().setValue resultString
